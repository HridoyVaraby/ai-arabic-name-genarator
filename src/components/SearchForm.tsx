  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-lg shadow-md">
      <div className="flex border-b mb-4">
        <button
          className={`px-2 py-2 sm:px-4 sm:py-2 text-xs sm:text-sm ${activeTab === 'letter' ? 'border-b-2 border-emerald-600 text-emerald-600' : 'text-gray-500'}`}
          onClick={() => setActiveTab('letter')}
        >
          Search by Letter
        </button>
        <button
          className={`px-2 py-2 sm:px-4 sm:py-2 text-xs sm:text-sm ${activeTab === 'meaning' ? 'border-b-2 border-emerald-600 text-emerald-600' : 'text-gray-500'}`}
        ...
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 sm:px-4 sm:py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm sm:text-base"
            >
              ...
              {(['male', 'female', 'neutral'] as const).map((option) => (
                <label key={option} className="flex items-center">
                  <input
                    ...
                    className="mr-1 sm:mr-2 text-emerald-600 focus:ring-emerald-500"
                  />
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
            <button
              ...
              className="flex items-center justify-center px-3 py-2 sm:px-4 sm:py-2 bg-emerald-600 text-white rounded-md hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 disabled:opacity-50 text-sm sm:text-base"
            >
              {isLoading ? (
                <RefreshCw className="w-4 h-4 sm:w-5 sm:h-5 animate-spin mr-1 sm:mr-2" />
              ) : (
            ...
            <button
              type="button"
              onClick={onReset}
              className="flex items-center justify-center px-3 py-2 sm:px-4 sm:py-2 border border-gray-300 rounded-md hover:bg-gray-50 text-sm sm:text-base"
            >
